...
<body>
<div id="wrap">
  <h3>Life evaluation over time</h3>
...
function renderFilter(){
  ...
  f.selectAll("span.chip").data(order,d=>d).join("span")
    .attr("class","chip").attr("title","Click to remove")
    .on("click",(e,d)=>{ state.active.delete(d);
      if(state.sel.country===d){
        const on=[...state.active.keys()];
